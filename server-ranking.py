...
flow_size = 100
BATCH_SIZE = 32
microbatches = 1            # it has to be 1 due to a bug
learn_rate = 0.0001
learn_rate_dp = 0.001
negative_samples = 1        # number of uncorrelated pairs for each correlated pair
NUM_PREDICTIONS = 10        # number of inferences
TRAIN = False               # if true, train the model, else use model weights checkpoints      
...
def load_model():
    ...
    EPOCH_COUNT = 10                # 30 for DP

    model = keras.Sequential()

    model.add(keras.layers.Flatten(batch_input_shape=[BATCH_SIZE, 8, flow_size, 1]))

    model.add(keras.layers.Dense(int(nnLayers[0]), kernel_initializer=keras.initializers.RandomNormal(mean=0.0, stddev=0.01, seed=None)))
    model.add(keras.layers.Activation('relu'))
    model.add(keras.layers.Dense(int(nnLayers[1]), kernel_initializer=keras.initializers.RandomNormal(mean=0.0, stddev=0.01, seed=None)))
    model.add(keras.layers.Activation('relu'))
    model.add(keras.layers.Dense(int(nnLayers[2]), kernel_initializer=keras.initializers.RandomNormal(mean=0.0, stddev=0.01, seed=None)))
    model.add(keras.layers.Activation('relu'))
    ...
    if dpsgd:        
        ...
        loss = customLoss_2
        checkpoint_path = "checkpoints/ranking/checkpoints_privacy_batch_size=" + str(BATCH_SIZE) + "/"

    else:
        optimizer = tf.keras.optimizers.Adam(learning_rate=learn_rate)
        loss = customLoss
        checkpoint_path = "checkpoints/ranking/checkpoints_server_batch_size=" + str(BATCH_SIZE) + "/"
    ...
    if TRAIN:
        ...
        if dpsgd:
            plot_name = 'plots/ranking/DP/loss_plot_batch_size_' + str(BATCH_SIZE) + '.png'
        else:
            plot_name = 'plots/ranking/without_DP/loss_plot_batch_size_' + str(BATCH_SIZE) + '.png'
        ...
        if dpsgd:
            plot_name_2 = 'plots/ranking/DP/accuracy_plot_batch_size_' + str(BATCH_SIZE) + '.png'
        else:
            plot_name_2 = 'plots/ranking/without_DP/accuracy_plot_batch_size_' + str(BATCH_SIZE) + '.png'
...
if __name__ == "__main__":
    ...
    with tfe.protocol.SecureNN():
        ...
        server = tfe.serving.QueueServer(input_shape=q_input_shape, output_shape=q_output_shape, computation_fn=model)
